...
nodeX <- c(H11=100, H21=200, H31=300, H12=100, H22=200, H32=300)
nodeY <- c(H11=200, H21=200, H31=200, H12=100, H22=100, H32=100)
...
As an example we now create the graph from \cite{bretzEtAl2009power}
...
Coordinates are interpretated as pixels in the GUI or 2 big points in {\LaTeX} (72 bp = 1 inch).
...
You can scale the TikZ graphic by changing the \texttt{[scale=0.7]}
option.  By default \texttt{graph2latex} scales down all TikZ graphic
to 70\% and has an optional parameter \texttt{scale}.
