...
(We are compatible to the renderInfo usage from package Rgraphviz - see e.g. \texttt{Rgraphviz:::layoutGraphviz}.)
...
Coordinates are interpretated as pixels in the GUI or big points in {\LaTeX} (72 bp = 1 inch).
...
The following command starts a graphical user interface for the
creation of SRMTP graphs.  Its only argument is the variable name for
the graph, given as a character string, under which the created
...
The GUI also allows you to update existing graphs.
Please note that the GUI is not yet functional for the 0.1 release of gsrmtp.
For release 0.2 a fully functional GUI is targeted.
...
We can take a look at the last graph (see figure \ref{finalstate}):
...
The command \texttt{createGsrmtpReport} can generate a full report:
...
The estimates are $\Sexpr{format(qt(0.02/2,df=9)*1.2/sqrt(10))}$, $\Sexpr{format(qt(0.055/2,df=9)*0.8/sqrt(10))}$ and $\Sexpr{format(qt(0.012/2,df=9)*1.7/sqrt(10))}$, 
the sample standard deviations $1.2$, $0.8$ and $1.7$ the t-statistics $\Sexpr{format(qt(0.02/2,df=9))}$, $\Sexpr{format(qt(0.055/2,df=9))}$ and $\Sexpr{format(qt(0.012/2,df=9))}$ and the corresponding p-values $p_1=0.02$, $p_2=0.055$ and $p_3=0.012$.
We want to adjust for multiple testing by using the Bonferroni-Holm-Procedure. TODO: This is not finished and nonsense so far!
...
simConfint(createBonferroniHolmGraph(3), c(0.02,0.055,0.012), function(node, alpha) {return(qnorm(alpha/2)*c(-1,1))})
