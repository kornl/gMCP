...
\newcommand{\al}{$\alpha-\textrm{level}$}
\newcommand{\eg}{\em e.g.}
\newcommand{\ie}{\em i.e.}
\newcommand{\gmcp}{\texttt{gMCP}}
\newcommand{\gact}{\texttt{gACT}}
\begin{document}

% \VignetteIndexEntry{Graphical approaches for multiple endpoint problems using weparametric tests}
...
This document describes how-to use \gmcp~to to construct, conduct and evaluate
...
inferences based on $z$-tests. Assuming normally distributed test
statistics with known variance one can improve on the Bonferroni test
in terms of power, provided that there exists (partial) knowledge of
the correlation structure between test statistics. If the correlation
matrix of a given subset of test statistics is known tests for
intersection hypotheses containing the associated elementary
hypotheses are computed using the multivariate distribution under the
respective null. The consonance condition providing the shortcut
utilized by sequentially rejective tests \cite{Hommel07,bretzEtAl2009graphical} can
no longer be guaranteed using this approach, hence, the whole closed
test has to be performed. For an exhaustive theoretical  specification
...
be made. For example consider the comparison of two treatments two a
...
global \al~level reserved for this treatment is passed to the other
...
We show in Section \ref{sec:commline} how these parameters can be
...
index which is passed to the elementary hypotheses associated with the
...
package \gmcp. The weights set for the graph can be extracted function
\texttt{graph2weights}. 
...
<<eval=F>>=
G <- matrix2graph(Gm,alpha=.025*w)
graphGUI(G)
@ 

\begin{figure}
  \centering
  \includegraphics[width=.95\textwidth]{pictures/FullFeaturedGUI.png}
...
elementary hypotheses, based on actual data, needs do be determined
...
matrix can currently be handled by \gmcp. In order to work we need to
know the whole correlation matrix for any subset of hypotheses, whose
bounds are to be computed using the multivariate distribution. For
...
<<>>=
p <- rnorm(1-c(2.24,2.24,2.24,2.3))
G <- matrix2graph(Gm,w*.025)
gMCP(G,p)
@ 
...
Example 2 is implemented by additionally passing the correlation
matrix:
...
which returns a vector specifying which of the elementary hypotheses
can be rejected at the overall \al.I
