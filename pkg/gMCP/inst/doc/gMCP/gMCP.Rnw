...
graph based multiple test procedures. By defining weighted graphs one
...
cases shortcuts ar   e available, for example the weighted Bonferroni
procedure leads to a sequentially rejective multiple test procedure.
...
\begin{description}
  ...
  \item[Use JLaTeXMath] There are few reasons not to use the free Java
  ...
  \item[Show fractions instead of decimal numbers]
  \item[Use epsilon approximation] Not yet available in this version.
  \item[Epsilon]
  \item[Try to show fractions / rounded numbers]
  \item[Number of digits to assure]
  \item[Verbose output of algorithms]
  \item[Monte Carlo sample size for power]
  \item[Type of random numbers]
  \item[Check online for updates]
  \item[Export images with transparent background]
