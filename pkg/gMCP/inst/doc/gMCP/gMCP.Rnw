...
\texttt{graphNEL}\footnote{The \texttt{initialize} method of this subclass
differs only in an extra argument \texttt{weights} for the nodes that
specifies the initial allocation of the significance level alpha to the individual
hypotheses.  Declaration of the nodes and edges is inherited from
class \texttt{graphNEL}.}.

The most convenient way to create a graph in R is to use the functions \texttt{matrix2graph} and \texttt{setWeights}.
As an example we create the graph from Bretz et
...
\section*{Symbolverzeichnis}\footnotesize
%\twocolumn[\section{Symbolverzeichnis}]
\begin{tabularx}{\textwidth}{lX}
\multicolumn{2}{l}{\textbf{Mengen}}\\
\R& Menge der reellen Zahlen\\
\NN& die natürlichen Zahlen mit 0\\
$\Pot(X)$ & Potenzmenge von $X$, d.h. Menge alle Teilmengen von $X$\\
\\
%\multicolumn{2}{l}{\textbf{Relationen}}\\
%$X\Subset Y$& $X$ ist relativ kompakt in $Y$\\
%\end{tabularx}\\
%\begin{tabularx}{\textwidth}{lX}
\multicolumn{2}{l}{\textbf{Abbildungen und Funktoren}}\\
$\skp{\cdot,\cdot}$ & das Standard-Skalarprodukt auf $\C^n$ mit $\skp{x,y}=\sum_{j=1}^nx_j\overline{y_j}$ für $x,y\in\C^n$ oder\\
$\id_X$ & Identität auf $X$\\
\\
\multicolumn{2}{l}{\textbf{Andere Symbole}}\\
$\hookrightarrow$& Abbildungspfeil, der zusätzlich die Injektivität der Abbildung anzeigt\\
$\twoheadrightarrow$& Abbildungspfeil, der zusätzlich die Surjektivität der Abbildung anzeigt\\
\end{tabularx}
%\onecolumn

\bigskip
Ist $a$ Element eines direkten Produktes $\prod_{j\in J}N_j$ von Mengen $N_j$, so bezeichnet $a_k:=\pi_k(a)\in N_k$ die $k$-te Komponente.
