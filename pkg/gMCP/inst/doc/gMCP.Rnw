...
\begin{Example}
  Let $p_1=0.01$, $p_2=0.04$ and $p_3=0.02$ be three p-values and
  ...
  $p_1<\alpha/2$.  Again the graph is updated and at last also $H_2$
  can be rejected.
\end{Example}
...
\subsubsection{Adjusted p-values and simultaneous confidence intervals}\index{adjusted p-values}
...
@
\end{tikzpicture}
  \caption{\label{gatekeeping} The Parallel Gate Keeping and the Improved Parallel Gate Keeping Procedure.}
\end{figure}
...
gMCP(graph, pvalues=c(0.02, 0.04, 0.01, 0.02))
...
\begin{figure}[ht]
  \centering    
  \includegraphics[width=0.7\textwidth]{pictures/fileMenu.png}      
...
\begin{figure}[ht]
  ...
  \caption{\label{pngExport} Graphics can be exported from the GUI to PNG files. Note that the checkerboard pattern in the background of some image viewers is just a way to visualize transparency.}
\end{figure}
