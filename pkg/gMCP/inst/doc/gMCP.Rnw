...
\begin{figure}[ht]
  ...
  \caption{\label{fullGUI} Graphics can be exported from the GUI to PNG files - the squared. Note that the checkerboard pattern in the background of some image viewers is just a way to visualize transparency.}
\end{figure}
