...
The GUI supports epsilon edges. You can enter the weights in R syntax, e.g.\ \texttt{1-2*e+1/3*e\^{}2} for $1-2e+\frac{1}{3}e^2$.
...
\subsection{Important TikZ commands for optimizing the reports}\index{graph2latex}
