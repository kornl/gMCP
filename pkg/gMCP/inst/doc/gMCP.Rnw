...
\title{gMCP - an R package for a graphical approach to sequentially
...
This package provides functions and graphical user interfaces for graph based multiple test procedures.
These graphs define a weighting strategy for all subsets of null hypotheses and following the closed test procedure weighted tests can be performed on these subsets.
In some case shortcuts are available, for example the weighted Bonferroni corrected tests lead to a sequentially rejective multiple test procedure.

At all steps either graphical user interfaces or the R Console, S4 objects and methods can be used.

Please note that this is still a beta release and the API will change in future versions.
...
We can even change the position of the edge labels for further fine tuning of the graphical representation.
With the following command we place the label for the edge from \texttt{H1} to \texttt{H2} at position (200, 80):
...
We can also constuct a graph from a given adjacency matrix via the command \texttt{matrix2graph}:
...
and editing MCP graphs that the \texttt{gMCP} package includes.
...
\texttt{graphMCP} object will be available from the R command line.
...
\includegraphics[width=0.5cm]{pictures/adjPval.png} Calculates the adjusted p-values.

\includegraphics[width=0.5cm]{pictures/confint2.png} Calculates simultaneous confidence intervals.

\includegraphics[width=0.5cm]{pictures/StartTesting.png} \includegraphics[width=0.5cm]{pictures/Reset.png} Starts the testing procedure / goes back to the graph modification. 
...
The command \texttt{gMCPReport} generates a full report of the testing procedure:
...
Use the following two buttons: \includegraphics[width=1cm]{pictures/adjPval_b.png} \includegraphics[width=1cm]{pictures/confint2_b.png}
...
and for complicated graphs the \texttt{gMCP} package will often fail to accomplish this.
