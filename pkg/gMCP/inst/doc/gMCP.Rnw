...
\begin{Theorem}[Bonferroni-Holm-Procedure] Let $T_1, \ldots, T_m$
be test statistics for $m\in\N$ null hypotheses $H_1, \ldots, H_m$ and $p_1, \ldots, p_m$ 
the associated p-values. Then the following test will control the familywise error rate at level $\alpha\in]0,1[$ in the strong sense:
...
The corresponding graph for the Bonferroni-Holm-Procedure for three hypotheses is given in Figure \ref{exampleHolm}.
We see a fully connected graph, where each node represents a hypothesis and the nodes and edges have weights.
...
alpha level of the corresponding node.
In this case the graph will be updated and the alpha level of this node
is passed according to the edge weights.   

\begin{Example}
  Let $p_1=0.01$, $p_2=0.04$ and $p_3=0.02$ be three p-values and $alpha=0.05$.
  In the first step $H_1$ can be rejected since $p_1<\alpha/3$.
  The updated graph can be seen in figure \ref{exampleHolmP}
  and now also $H_3$ can be rejected since $p_1<\alpha/2$.
  Again the graph is updated and at last also $H_2$ can be rejected.
\end{Example}
...
@
  \caption{\label{exampleHolmP} Example showing how all three
    null hypotheses can be rejected with p-values $p_1=0.01$,
    $p_2=0.04$ and $p_3=0.02$.}
\end{figure}

Okay, let's reproduce this with the \texttt{gMCP} package. We start R and enter:
...
The GUI seen in Figure \ref{fullGUI} is shown and we select from the menu "Example graphs" the entry "Bonferroni-Holm Test".
We enter the three p-values in the respective fields on the right side.
By clicking on the button with the green arrow we start the test procedure
and can sequentially reject all three hypotheses.  
