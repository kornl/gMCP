...
\subsection{Two types of correlation matrices}\hypertarget{cormat2}{}
...
\end{Theorem}
\begin{Rem}
It is not possible to calculate adjusted p-values for the timmed weighted Simes test, since for example with $w_A=w_B=0.5$, $p_A=0.01$, and $p_A=0.97$ the test does reject $H_0$ for $\alpha=0.05$ since $p_A \leq w_A\cdot\alpha=0.025$ and $p_B<1-w_B\cdot\alpha=0.975$, but not for $\alpha=0.1$ since $p_B>1-w_B\cdot\alpha$ in this case.
\end{Rem}
...
\subsubsection*{Adjusted p-Values in the Simes Test}%self derived, no source
