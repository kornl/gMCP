...
The underlying distribution of test statistics in power simulations is assumed to be multivariate normal in the following with the exception of section \ref{non-normal} "Simulation for Non-normal Distributions".
...
In the following let $A_i$ be the sample sizes ratio of arm $i$ to the first arm (with $A_1$ always equal to 1) and 
...
\[\frac{\mu_i-\mu_j}{\sigma}/\sqrt{\frac{1}{N}\left(\frac{1}{r_i}+\frac{1}{r_j}\right)}\text{ and a standardized effect size }\text{ES}=\frac{\mu_i-\mu_j}{\sigma}.\]

\subsection{Power Requirements}

\begin{figure}[ht]
  \centering    
  \includegraphics[width=9cm]{pictures/power_requirements.png}      
  \caption{\label{powerReq} ...}
\end{figure}

\section{Simulation for Non-normal Distributions}\label{non-normal}

If the underlying distribution of the test statistics is not multivariate normal, but test statistics can be simulated and p-values generated, one can still use function graphTest:
...
rho <- 0.6
n <- 30
cr <- matrix(rho, ncol=4, nrow=4) + diag(rep(1-rho, 4))
pvals <- t(replicate(1e2, 
                     sapply(colSums(rmvbin(n, margprob = c(0.45, 0.4, 0.3, 0.25), bincorr = cr)), 
                            function(x, ...) {binom.test(x, ...)$p.value}, n=n)
                     ))
# Other example:
...
count <-0
for (g in graphs) {
  cat(graph2latex(g, fig=TRUE, scaleText=TRUE, scale=0.7, fig.caption=texify(attr(g, "descr"))))
  #cat(attr(g, "descr"))
  count <- count + 1
...
\chapter{Appendix - Multiple Testing Basics}
...
For two one-sided tests where the bivariate test statistic fulfills certain conditions, \cite{brannath2009trimmed} show that the following trimmed version of the weighted Simes test is bounded by $\alpha$ und $H_0$:
$H_A\cap H_B$ is rejected if
\begin{itemize}
\item $p_A \leq \alpha_A$ and $p_B<1-\alpha_B$ or
\item $p_B \leq \alpha_B$ and $p_A<1-\alpha_A$ or
