...
\maketitle\thispagestyle{empty}\vspace*{4cm}
...
of the graph according to Algorithm \ref{rmNodeAlg} (the order does not matter).
For a more detailed version please take a look at the article from \cite{Bretz11} 
that is freely available as OnlineOpen Article.
...
cat(graph2latex(graph, scale=0.7, labelTikZ="near start,fill=blue!20"))
@
...
<<echo=FALSE,results='asis'>>=
graph <- BonferroniHolm(3)
cat(graph2latex(graph, scale=0.7, nodeTikZ="minimum size=1.2cm"))
...
<<echo=FALSE,results='asis'>>=
graph <- BretzEtAl2011()
cat(graph2latex(graph, scale=0.7))
@
  \caption{\label{exampleGraphBretz} Example graph from \cite{bretz2011test} that we will create in this vignette.}
\end{figure}
...
\ref{exampleGraphBretz} shows the compiled result):
...
\begin{figure}[ht]
  \centering   
<<echo=FALSE,results='asis'>>=
cat(graph2latex(result@graphs[[4]], scale=0.7))
@
...
\ref{correlated}). Either you can perform a Simes test or a weighted parametric
...
\begin{figure}[ht]
  ...
  \caption{\label{createCM1} Dialog for specifying a correlation matrix.}
\end{figure}
...
cat(graph2latex(parallelGatekeeping(), scale=0.7, nodeTikZ="minimum size=1.2cm", tikzEnv=FALSE))
cat(graph2latex(improvedParallelGatekeeping(), scale=0.7, nodeTikZ="minimum size=1.2cm", tikzEnv=FALSE, offset=c(300, 0)))
...
We support entangled graphs as a new feature. But since it is not yet well
tested, you have to activate it in the options (see \ref{experimentalFeatures}).
...
\node (H1) at (70bp,-70bp)[draw,circle split,fill=green!80] {$H1$ \nodepart{lower} $\frac\alpha2\;0$};
\node (H2) at (210bp,-70bp)[draw,circle split,fill=green!80] {$H2$ \nodepart{lower} $\frac\alpha2\;0$};
\node (H3) at (140bp,-140bp)[draw,circle split,fill=green!80] {$H3$ \nodepart{lower} $0\;0$};
\node (H4) at (70bp,-210bp)[draw,circle split,fill=green!80] {$H4$ \nodepart{lower} $0\;0$};
\node (H5) at (210bp,-210bp)[draw,circle split,fill=green!80] {$H5$ \nodepart{lower} $0\;0$};
\draw [draw=black,->,line width=1pt] (H1) to[bend left=15] node[midway,above,fill=blue!20] {$1$} (H3);
\draw [draw=black,->,line width=1pt] (H2) to[bend left=15] node[midway,above,fill=blue!20] {$1$} (H3);
\draw [draw=black,->,line width=1pt] (H3) to[bend left=15] node[midway,above,fill=blue!20] {$1-\epsilon$} (H4);
\draw [draw=black,->,line width=1pt] (H3) to[bend left=15] node[midway,above,fill=blue!20] {$\epsilon$} (H5);
\draw [draw=black,->,line width=1pt] (H4.135) arc(-120:-310:100bp) node[fill=blue!20] {$1$} to (H2);
\draw [draw=blue,->,line width=1pt] (H1) to[bend right=15] node[midway,above,fill=blue!20] {$1$} (H3);
\draw [draw=blue,->,line width=1pt] (H2) to[bend right=15] node[midway,above,fill=blue!20] {$1$} (H3);
\draw [draw=blue,->,line width=1pt] (H3) to[bend right=15] node[midway,above,fill=blue!20] {$\epsilon$} (H4);
\draw [draw=blue,->,line width=1pt] (H3) to[bend right=15] node[midway,above,fill=blue!20] {$1-\epsilon$} (H5);
\draw [draw=blue,->,line width=1pt] (H5.45) arc(-60:130:100bp) node[fill=blue!20] {$1$} to (H1);
\end{tikzpicture}

%<<echo=FALSE,results=tex>>=
%
%cat(graph2latex(Entangled1Maurer2012(), scale=0.7))
%
%@
...
cat(graph2latex(generalSuccessive(), scale=0.7))
...
%  differ in more than the specified value from the floating point number.
  ...
  \item[Weights of subgraphs are upscaled to 1] If "yes" is selected the weights 
    of all subgraphs are always upscaled to one as described in \cite{Bretz11}.
...
cat(graph2latex(result@graphs[[3]], pvalues=pvalues, scale=0.7))
...
Adjusted p-values form the Hochberg Procedure are greater as or equal to the Hommel-adjusted p-values.   
