...
cat(graph2latex(G, fig=FALSE, scaleText=TRUE, scale=0.8, fig.caption="Bonferroni-Holm"))
@
\end{center}
...
for (i in 1:m) {
  if (mu[i]>0) {
    ...
    for (j in 1:m) {
      if (M[i, j]!=0) {
        #M[i, j] <- paste(gMCP:::getLaTeXFraction(as.numeric(M[i,j])), "(1-q^{\\mu})", sep="\cdot")
        M[i, j] <- paste(M[i,j], "(1-q^{\\mu})", sep="*")
      }
    }
    M[i, dim(M)[1]] <- "q^{\\mu}"
    G@m <- M
