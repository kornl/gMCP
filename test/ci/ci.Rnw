...
getV <- function(G, mu, q) {
  G2 <- replaceVariables(G, variables = list(q=q, mu="mu"), partial=TRUE, envir=list(mu=mu))
  print(G2)
...
rightside <- Vectorize(function(mu, q=0.5, alpha=0.05) {
  mu=mu+c(-1,2,3)
  Gmu <- createGraph(G, mu=mu)
  V <- getV(G=Gmu, mu=mu, q=q)
  return(q^mu[i]*V[i]*alpha)
})

i<-1
plot(rightside, xlim=c(-4,4))
plot(p[[i]], xlim=c(-4,4), add=TRUE, col="blue")
i<-2
plot(rightside, xlim=c(-4,4))
plot(p[[i]], xlim=c(-4,4), add=TRUE, col="blue")
i<-3
